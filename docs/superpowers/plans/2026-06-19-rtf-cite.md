...
def test_multi_reference_marker_renders_as_one_group(tmp_path, capsys):
    ...
    text = out.read_text(encoding="latin-1")
    # A comma-joined run of numbers inside a superscript indicates grouping.
    import re as _re
    assert _re.search(r"\\super[^\\]*\d+\s*[,–-]\s*\d+", text), \
...
- **Grouped multi-citations:** all references in one marker render as a single
  collapsed citation (superscript `1,2,3` / `[1,2,3]`, never `[1][2][3]`),
  pinned by `test_multiple_refs_go_in_one_group` (T4) and
  `test_multi_reference_marker_renders_as_one_group` (T7).
